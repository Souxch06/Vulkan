...
-- Anti-Exploit System Removed - No longer needed
...
-- Anti-Exploit system completely removed

-- Anti-Exploit functions removed

-- Spy text detector removed

--======================= Remote Hook Detection ===========================--

-- All anti-exploit functions removed

-- Dex/Explorer detection removed

-- Print hook removed

-- Anti-exploit bootstrap removed
...
local CONFIG = {
    ...
    Movement = {
        ...
        JumpBoost = {
            Enabled = false,
            JumpPower = 100, -- Enhanced jump power
        },
        HeightBypass = {
            Enabled = false,
            IntermediateSize = { X = 2, Y = 20, Z = 1 },
        ...
        FloatToPlot = {
            Enabled = false,
            Speed = 50, -- Speed to move to plot
            MaxDistance = 1000, -- Maximum distance to search for plots
        },
        Freeze = {
            Enabled = false,
            GroundPartSize = Vector3.new(100, 1, 100),
            WeldOffset = Vector3.new(0, -3, 0),
        },
        Helicopter = {
            Enabled = false,
...
_G.applyLoadedToggleStates = function()
    pcall(function()
        ...
        task.delay(2.0, function() -- Increased delay to ensure character is fully ready
            ...
            -- Invisibility auto-start removed to prevent unwanted activation
...
-- Main Frame (with comprehensive error handling)
local mainFrame
pcall(function()
...
-- Settings UI (with error handling)
local settingsFrame
pcall(function()
...
-- Section Frames (with error handling)
local sections = {}
local function createSection(name)
    local sectionFrame
    local success, err = pcall(function()
        sectionFrame = Instance.new("ScrollingFrame")
    ...
    if not success then
        warn("Failed to create section " .. name .. ": " .. tostring(err))
        return nil
    end
...
local function createBillboardGui(plr, char)
    local success, billboard, distanceLabel, iconFrame = pcall(function()
        ...
        gui.StudsOffset = Vector3.new(0, 6, 0) -- Raised higher to avoid stacking with other ESP
        ...
        usernameLabel.Size = UDim2.new(1, 0, CONFIG.ESP.PlayerESP.ShowDistance and 0.35 or 1, 0)
        ...
        iconFrame.Position = UDim2.new(0, 0, CONFIG.ESP.PlayerESP.ShowDistance and 0.65 or 0.35, 0)
...
local function createPlotBillboardGui(plot)
    local success, billboard, distanceLabel, ownerLabel, timeLabel = pcall(function()
        ...
        gui.SizeOffset = Vector2.new(0, 0)
        gui.StudsOffset = Vector3.new(0, 4, 0) -- Medium height for Plot ESP
        gui.AlwaysOnTop = true
        gui.MaxDistance = 10000
...
local function enablePlotTimeESP()
    ...
    local success, _ = pcall(function()
        ...
        _G.PlotTimeESP_Data.playerAddedConn = Players.PlayerAdded:Connect(function(plr)
            ...
            task.spawn(function()
                task.wait(0.5) -- Shorter wait for faster response
            for plot, data in pairs(_G.PlotTimeESP_Data) do
                ...
                -- Also trigger a full refresh to ensure accuracy
                _G.refreshPlotTimeESP()
            end)
        end)
        
        _G.PlotTimeESP_Data.playerRemovingConn = Players.PlayerRemoving:Connect(function(plr)
            -- Update all plot time billboards when a player leaves (immediate)
            for plot, data in pairs(_G.PlotTimeESP_Data) do
                if typeof(plot) == "Instance" and data.billboard and data.timeLabel then
                    pcall(function()
                    updatePlotTimeBillboard(plot, data)
        ...
        -- Handle character spawning for existing players
        for _, existingPlayer in ipairs(Players:GetPlayers()) do
            if existingPlayer.Character then
                _G.PlotTimeESP_Data.playerCharacterAddedConn = existingPlayer.CharacterAdded:Connect(function(character)
                    task.spawn(function()
                        task.wait(0.5) -- Shorter wait for faster response
                        for plot, data in pairs(_G.PlotTimeESP_Data) do
                            if typeof(plot) == "Instance" and data.billboard and data.timeLabel then
        ...
        -- Also connect to future players' character spawning
        _G.PlotTimeESP_Data.futurePlayerCharacterConn = Players.PlayerAdded:Connect(function(plr)
        ...
        -- Periodic refresh to ensure timers stay accurate
        _G.PlotTimeESP_Data.periodicRefreshConn = RunService.Heartbeat:Connect(function()
            if not _G.PlotTimeESP_Enabled then return end
            -- Refresh all plot time billboards every 3 seconds (very frequent)
            if not _G.PlotTimeESP_Data.lastPeriodicRefresh then
                _G.PlotTimeESP_Data.lastPeriodicRefresh = 0
            end
            _G.PlotTimeESP_Data.lastPeriodicRefresh = _G.PlotTimeESP_Data.lastPeriodicRefresh + 1/60
            if _G.PlotTimeESP_Data.lastPeriodicRefresh >= 3 then -- 3 seconds
                _G.PlotTimeESP_Data.lastPeriodicRefresh = 0
                for plot, data in pairs(_G.PlotTimeESP_Data) do
                    if typeof(plot) == "Instance" and data.billboard and data.timeLabel then
                        pcall(function()
...
-- Jump Boost Control
--=========================================================
local function setupJumpBoostControl(parent)
    local jumpBoostData = {
        defaultJumpPower = 50,
        isActive = false
    }

    local jumpBoostSwitch = createSwitch(parent, "Jump Boost", _G.SavedToggleStates and _G.SavedToggleStates.JumpBoost or false, function(on)
        local success, _ = pcall(function()
            jumpBoostData.isActive = on
            if humanoid then
                humanoid.UseJumpPower = true
                humanoid.JumpPower = on and CONFIG.Movement.JumpBoost.JumpPower or jumpBoostData.defaultJumpPower
            end
            if on then
                -- Auto-create side toggle when enabled
                _G.createCircularToggleUI("JumpBoost", function() return jumpBoostSwitch.get() end, function(state) jumpBoostSwitch.set(state) end)
            else
                -- Remove side toggle when disabled
                local existingToggle = _G.circularToggleGui:FindFirstChild("JumpBoostToggleUI")
                if existingToggle then
                    _G.OpenCircularToggles["JumpBoost"] = nil
                    existingToggle:Destroy()
                    _G.saveSettings()
                end
            end
        ...
        if not success then
            warn("Failed to toggle jump boost")
        end
    end)

    return jumpBoostData.isActive, jumpBoostSwitch
end

-- Setup Movement Controls (with error handling)
local jumpSwitch, speedSwitch, jumpBoostSwitch
pcall(function()
    _, jumpSwitch = setupJumpPowerControl(_G.movementSection)
    _, speedSwitch = setupSpeedControl(_G.movementSection)
    _, jumpBoostSwitch = setupJumpBoostControl(_G.movementSection)
end)

--=========================================================
-- Height Bypass Control
--=========================================================
local function setupHeightBypassControl(parent)
    local defaultSize = Vector3.new(2, 2, 1)
    local isHeightBypassActive = false
    local heightBypassThread = nil

    local heightBypassSwitch = createSwitch(parent, "Height Bypass", _G.SavedToggleStates and _G.SavedToggleStates.HeightBypass or false, function(on)
        local success, _ = pcall(function()
            isHeightBypassActive = on
            if not humanoidRootPart then return end
            if on then
                if not heightBypassThread then
                    heightBypassThread = task.spawn(function()
                        while isHeightBypassActive do
                            if humanoidRootPart then
                                humanoidRootPart.Size = Vector3.new(
                                    CONFIG.Movement.HeightBypass.IntermediateSize.X,
                                    CONFIG.Movement.HeightBypass.IntermediateSize.Y,
                                    CONFIG.Movement.HeightBypass.IntermediateSize.Z
                                )
                                task.wait(0.2)
                                if not isHeightBypassActive then break end
                                humanoidRootPart.Size = Vector3.new(
                                    CONFIG.Movement.HeightBypass.TallSize.X,
                                    CONFIG.Movement.HeightBypass.TallSize.Y,
                                    CONFIG.Movement.HeightBypass.TallSize.Z
                                )
                                task.wait(2.1)
                                if not isHeightBypassActive then break end
                        ...
                        heightBypassThread = nil
                    end)
                end
                -- Auto-create side toggle when enabled
                _G.createCircularToggleUI("HeightBypass", function() return heightBypassSwitch.get() end, function(state) heightBypassSwitch.set(state) end)
            else
                if humanoidRootPart then
                    humanoidRootPart.Size = defaultSize
                end
                if heightBypassThread then
                    task.cancel(heightBypassThread)
                    heightBypassThread = nil
                end
                -- Remove side toggle when disabled
                local existingToggle = _G.circularToggleGui:FindFirstChild("HeightBypassToggleUI")
                if existingToggle then
                    _G.OpenCircularToggles["HeightBypass"] = nil
                    existingToggle:Destroy()
                    _G.saveSettings()
                end
            end
    ...
    return isHeightBypassActive, heightBypassSwitch
end
...
--=========================================================
-- Float to Plot Control
--=========================================================
local function setupFloatToPlotControl(parent)
    local isFloatToPlotActive = false
    local floatToPlotConnection = nil

    local floatToPlotSwitch = createSwitch(parent, "Float to Plot", _G.SavedToggleStates and _G.SavedToggleStates.FloatToPlot or false, function(on)
        local success, _ = pcall(function()
            isFloatToPlotActive = on
            if on then
                if not floatToPlotConnection then
                    floatToPlotConnection = RunService.Heartbeat:Connect(function()
                        if isFloatToPlotActive and humanoidRootPart then
                            -- Find the nearest plot
                            local nearestPlot = nil
                            local nearestDistance = CONFIG.Movement.FloatToPlot.MaxDistance
                            
                            for _, obj in pairs(workspace:GetChildren()) do
                                if obj.Name:find("Plot") and obj:IsA("Model") then
                                    local plotPosition = obj:GetBoundingBox().Position
                                    local distance = (humanoidRootPart.Position - plotPosition).Magnitude
                            ...
                            if nearestPlot then
                                local plotPosition = nearestPlot:GetBoundingBox().Position
                                local direction = (plotPosition - humanoidRootPart.Position).Unit
                                local moveVector = direction * CONFIG.Movement.FloatToPlot.Speed
                ...
                -- Auto-create side toggle when enabled
                _G.createCircularToggleUI("FloatToPlot", function() return floatToPlotSwitch.get() end, function(state) floatToPlotSwitch.set(state) end)
            else
                if floatToPlotConnection then
                    floatToPlotConnection:Disconnect()
                    floatToPlotConnection = nil
                end
                -- Remove side toggle when disabled
                local existingToggle = _G.circularToggleGui:FindFirstChild("FloatToPlotToggleUI")
                if existingToggle then
                    _G.OpenCircularToggles["FloatToPlot"] = nil
                    existingToggle:Destroy()
                    _G.saveSettings()
                end
            end
        ...
        if not success then
            warn("Failed to toggle float to plot")
        end
    end)

    return isFloatToPlotActive, floatToPlotSwitch
end
...
--=========================================================
-- Freeze Control (Renamed from Carpet Bypass)
--=========================================================
local function setupFreezeControl(parent)
    ...
    local freezeSwitch = createSwitch(parent, "Freeze", _G.SavedToggleStates and _G.SavedToggleStates.Freeze or false, function(on)
        local success, _ = pcall(function()
            isFreezeActive = on
            if on then
                ...
                if humanoidRootPart then
                    ...
                    print("Position saved for Freeze")
                else
                    warn("No character or HumanoidRootPart found")
                    return
                end
                
                -- Create invisible ground part to act as ground
                groundPart = Instance.new("Part")
                groundPart.Name = "FreezeGround"
                groundPart.Size = CONFIG.Movement.Freeze.GroundPartSize
                groundPart.Position = humanoidRootPart.Position + CONFIG.Movement.Freeze.WeldOffset
                ...
                -- Weld player to ground part to make them act like they're the ground
                weldConstraint = Instance.new("WeldConstraint")
                weldConstraint.Part0 = humanoidRootPart
                weldConstraint.Part1 = groundPart
                weldConstraint.Parent = humanoidRootPart
                
                -- Set up freeze loop
                freezeConnection = RunService.Heartbeat:Connect(function()
                    if isFreezeActive and savedCFrame and humanoidRootPart then
                        -- Use BodyPosition for stable positioning
                        ...
                        bodyPosition.Position = savedCFrame.Position
                        
                        -- Also use BodyAngularVelocity to maintain rotation
                        ...
                        -- CFrame teleport as backup (with nil check)
                        if savedCFrame then
                            for i=1,12 do
                                pcall(function()
                ...
                -- Auto-create side toggle when enabled
                _G.createCircularToggleUI("Freeze", function() return freezeSwitch.get() end, function(state) freezeSwitch.set(state) end)
                print("Freeze enabled - welded to ground")
            else
                ...
                -- Remove side toggle when disabled
                local existingToggle = _G.circularToggleGui:FindFirstChild("FreezeToggleUI")
                if existingToggle then
                    _G.OpenCircularToggles["Freeze"] = nil
                    existingToggle:Destroy()
                    _G.saveSettings()
                end
                
                print("Freeze disabled - unwelded from ground")
            end
        end)
        if not success then
            warn("Failed to toggle freeze")
        end
    end)

    return isFreezeActive, freezeSwitch
end
        ...
        local success, err = pcall(function()
            ...
            flingConnection = RunService.Heartbeat:Connect(function()
                if isFlingActive and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
                    ...
                    -- Spoof CFrame and velocity (with nil checks)
                    if desyncState[1] then
                        ...
                        if player.Character and hrp.Parent == player.Character and desyncState[1] then
                            pcall(function()
...
-- Setup Additional Movement Controls (with error handling)
local unhittableSwitchInstance, resizeSwitchInstance, heightBypassSwitchInstance, floatToPlotSwitchInstance, freezeSwitchInstance
pcall(function()
    _, unhittableSwitchInstance = setupUnhittableControl(_G.movementSection)
    _, resizeSwitchInstance = setupResizeControl(_G.movementSection)
    _, heightBypassSwitchInstance = setupHeightBypassControl(_G.movementSection)
    _, floatToPlotSwitchInstance = setupFloatToPlotControl(_G.movementSection)
    _, freezeSwitchInstance = setupFreezeControl(_G.movementSection)
end)
...
player.CharacterAdded:Connect(function(newCharacter)
    ...
    task.spawn(function()
        ...
        -- Reinitialize Plot Time ESP to update timers
        if _G.PlotTimeESP_Enabled then
            print("🔄 Reinitializing Plot Time ESP on character respawn...")
            pcall(function()
                disablePlotTimeESP()
                task.wait(0.5)
                enablePlotTimeESP()
...
-- ESP Update Interval Control
createNumberInput(settingsContent, "ESP Update Interval", CONFIG.ESP.UpdateInterval, function(value)
    local success, _ = pcall(function()
        CONFIG.ESP.UpdateInterval = math.clamp(value, 0.05, 1)
        _G.saveSettings()
        -- Force immediate update of all ESP systems
        if _G.ESP_Enabled then
            for plr, data in pairs(_G.ESP_Data) do
                if typeof(plr) == "Instance" and data.updateConn then
                    data.updateConn:Disconnect()
                    local lastUpdate = 0
                    data.updateConn = RunService.Heartbeat:Connect(function(deltaTime)
                        if not _G.ESP_Enabled or not _G.ESP_Data[plr] then return end
                        lastUpdate = lastUpdate + deltaTime
                        if lastUpdate >= CONFIG.ESP.UpdateInterval then
                            updateBillboard(plr, _G.ESP_Data[plr])
                            lastUpdate = 0
        ...
        if _G.PlotESP_Enabled then
            for plot, data in pairs(_G.PlotESP_Data) do
                if typeof(plot) == "Instance" and data.updateConn then
                    data.updateConn:Disconnect()
                    local lastUpdate = 0
                    data.updateConn = RunService.Heartbeat:Connect(function(deltaTime)
                        if not _G.PlotESP_Enabled or not _G.PlotESP_Data[plot] then return end
                        lastUpdate = lastUpdate + deltaTime
                        if lastUpdate >= CONFIG.ESP.UpdateInterval then
                            updatePlotBillboard(plot, _G.PlotESP_Data[plot])
                            lastUpdate = 0
    ...
    if not success then
        warn("Failed to update ESP Update Interval")
    end
end)
...
createSwitch(settingsContent, "Show Player Distance", CONFIG.ESP.PlayerESP.ShowDistance, function(on)
    local success, _ = pcall(function()
        ...
        _G.saveSettings()
        -- Recreate all player ESP billboards immediately
        if _G.ESP_Enabled then
            for plr, data in pairs(_G.ESP_Data) do
                if typeof(plr) == "Instance" and data.billboard then
                    ...
                    if newBillboard then
                        ...
                        print("🔄 Updated Player ESP Distance for: " .. plr.Name)
                    end
                end
            end
        end
            ...
            for plr, data in pairs(_G.ESP_Data) do
                if typeof(plr) == "Instance" and data.billboard then
                    ...
                    if newBillboard then
                        ...
                        data.distanceLabel = newDistanceLabel
                        data.iconFrame = newIconFrame
                        print("🔄 Updated Player ESP Items for: " .. plr.Name)
                    end
                end
            end
        end
...
-- Plot Time Size Slider removed for mobile compatibility
...
-- Reset a BillboardGui back to default (optimized)
_G.resetBillboard = function(billboard)
    if billboard then
...
-- Check if any ancestor is named "Base" (optimized)
_G.hasBaseParent = function(obj)
    _G.currentParent = obj.Parent
    while _G.currentParent do
        if _G.currentParent.Name == "Base" then
            return true
        end
        _G.currentParent = _G.currentParent.Parent
    end
    return false
end
...
local function updateBrainrotESP()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BillboardGui") and obj.Name == "AnimalOverhead" and _G.hasBaseParent(obj) then
...
local function createPlotTimeBillboard(plot)
    local success, billboard, timeLabel = pcall(function()
        ...
        gui.Adornee = spawnPart
        gui.Size = UDim2.new(0, 48, 0, 48) -- Fixed 48x48 size for mobile compatibility
        gui.SizeOffset = Vector2.new(0, 0)
        gui.StudsOffset = Vector3.new(0, 2, 0) -- Lower height for Plot Time ESP to avoid stacking
        gui.AlwaysOnTop = true
        gui.MaxDistance = 10000
        gui.Parent = spawnPart
        ...
        timeLabel.TextScaled = true -- Scale text to fit the 48x48 area
        timeLabel.TextSize = 12 -- Fixed text size
        timeLabel.Font = Enum.Font.SourceSansBold -- Bold font
        timeLabel.TextStrokeTransparency = 0.2 -- Subtle text stroke for readability
...
_G.refreshPlotTimeESP = function()
    if not _G.PlotTimeESP_Enabled then return end
    local success, _ = pcall(function()
        -- Clear existing data
        for plot, data in pairs(_G.PlotTimeESP_Data) do
...
local function disablePlotTimeESP()
    ...
    local success, _ = pcall(function()
        ...
        if _G.PlotTimeESP_Data.periodicRefreshConn then
            pcall(function() _G.PlotTimeESP_Data.periodicRefreshConn:Disconnect() end)
            _G.PlotTimeESP_Data.periodicRefreshConn = nil
        end
        if _G.PlotTimeESP_Data.futurePlayerCharacterConn then
            pcall(function() _G.PlotTimeESP_Data.futurePlayerCharacterConn:Disconnect() end)
            _G.PlotTimeESP_Data.futurePlayerCharacterConn = nil
        end
        for plot, data in pairs(_G.PlotTimeESP_Data) do
            if typeof(plot) == "Instance" then
                if data.plotConn then
...
-- Duplicate function removed - already defined above as global
...
local function disableBrainrotESP()
    ...
    if _G.brainrotLastHighlighted then
        _G.resetBillboard(_G.brainrotLastHighlighted)
        _G.brainrotLastHighlighted = nil
    end
end

-- Mobile Desync Functions (optimized to reduce local variables)
_G.enableMobileDesync = function()
    pcall(function()
        _G.mobilePlayer = _G.Services.Players.LocalPlayer
        _G.mobileCharacter = _G.mobilePlayer.Character or _G.mobilePlayer.CharacterAdded:Wait()
        _G.mobileHumanoid = _G.mobileCharacter:WaitForChild("Humanoid")
        _G.mobileHumanoidRootPart = _G.mobileCharacter:WaitForChild("HumanoidRootPart")

        _G.executeAntiHit = function()
        ...
        _G.executeAntiHit()
        print("✅ Mobile Desync activated (No Clone Method)!")
    end)
end

_G.disableMobileDesync = function()
    pcall(function()
        ...
        print("❌ Mobile Desync disabled - reset to default!")
    end)
end
...
createButton(_G.togglesSection, "Toggle Mobile Desync", function()
    createCircularToggleUI("Mobile Desync", function() return _G.mobileDesyncEnabled end, function(state)
        ...
        if state then
            _G.enableMobileDesync()
        else
            _G.disableMobileDesync()
        end
    end)
end)
...
-- Plot Time Size Slider mouse events removed for mobile compatibility
...
-- New Features Settings Controls
createNumberInput(settingsContent, "Jump Boost Power", CONFIG.Movement.JumpBoost.JumpPower, function(value)
    local success, _ = pcall(function()
        CONFIG.Movement.JumpBoost.JumpPower = math.clamp(value, 50, 200)
        _G.saveSettings()
    end)
    if not success then
        warn("Failed to update Jump Boost Power")
    end
end)

createNumberInput(settingsContent, "Float to Plot Speed", CONFIG.Movement.FloatToPlot.Speed, function(value)
    local success, _ = pcall(function()
        CONFIG.Movement.FloatToPlot.Speed = math.clamp(value, 10, 100)
        _G.saveSettings()
    end)
    if not success then
        warn("Failed to update Float to Plot Speed")
    end
end)

createNumberInput(settingsContent, "Float to Plot Max Distance", CONFIG.Movement.FloatToPlot.MaxDistance, function(value)
    local success, _ = pcall(function()
        CONFIG.Movement.FloatToPlot.MaxDistance = math.clamp(value, 100, 5000)
        _G.saveSettings()
    end)
    if not success then
        warn("Failed to update Float to Plot Max Distance")
    end
end)
...
resetSettingsButton.MouseButton1Click:Connect(function()
    local success, _ = pcall(function()
        ...
        for plot, data in pairs(_G.PlotESP_Data) do
            ...
            -- Plot Time ESP uses fixed text size for mobile compatibility
        end
...
--=========================================================
-- Character Respawn Handling (with comprehensive error handling)
--=========================================================
player.CharacterAdded:Connect(function(newCharacter)
    local success, err = pcall(function()
        character = newCharacter
    ...
    if not success then
        warn("Failed to handle character respawn: " .. tostring(err))
    end
end)
...
local function initialize()
    local success, err = pcall(function()
        ...
        if sections.Home then
            sections.Home.Visible = true
        end
        if sidebar and sidebar:FindFirstChild("HomeButton") then
